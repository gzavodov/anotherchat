...
class ChatServer implements MessageComponentInterface
{
	...
	public function onMessage(ConnectionInterface $socket, $data) 
	{
		...
		try
		{
			$addresser = $this->findClientByConnection($socket);
			...
			switch ($msg->topic) 
			{
				...
				case 'new_message':
				{
					...
					$message = Message::create(
						[
							'from_id' => $addresser->user->id,
							'to_id'   => $addresseeId,
							'message' => $msg->data->message
						]
					);
					
					$notification = [ 'topic' => 'messages', 'data' => [ $message ] ];
					$addresser->send($notification);
					if($addressee)
					{
						$addressee->send($notification);		
					}
					
					$this->sendMessageToAdmin($notification);
					break;
				}
				case 'request':
				{
						...
						if($method === 'delete' && $messageId > 0)
						{
							...
							if($message !== null)
							{
								...
								$this->sendMessageToUsers(
									[ $addresser->user->id, $message->from_id, $message->to_id ],
									[ 'topic' => 'deletion', 'data' => [ 'id' => $messageId ] ]
								);
							}
						}
						return;
					...
					if($scope === 'global' && $addresser->user->isAdmin())
					{
						$filter = array('scopeUserId' => $targetUserId);
					}
					else //if($scope === 'personal')
					{
						$filter = array('scopeUserId' => $addresser->user->id, 'targetUserId' => $targetUserId);
					}
					$this->sendClientMessageLog($addresser, $filter);
					break;
				}		
			}
		} 
	...
	protected function sendMessageToUsers(array $userIds, array $message)
	{
		$userMap = array_flip($userIds);
		foreach ($this->clients as $client)
		{
			if ($client->isLoggedIn() && isset($userMap[$client->user->id]))
			{
				$client->send($message);
			}
		}
	...
	protected function sendMessageToAdmin(array $message)
	{
		//NOTE: Suppose there is only one administrator
		foreach ($this->clients as $client)
		{
			if ($client->isLoggedIn() && $client->user->isAdmin())
			{
				$client->send($message);
				break;
			}
